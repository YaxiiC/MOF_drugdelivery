...
with open("mlp_skipped_cifs.log", "w") as f:
...
# =========================
print("\n===== 5-fold Cross Validation (MLP on handcrafted features) =====")
...
for train_idx, val_idx in kfold.split(X_feats):
    ...
    # Scale features for MLP (neural networks benefit from scaled features)
    ...
    y_pred = mlp_cv.predict(X_va_scaled)
...
# =========================
# 5. Define non-DGL model: MLP regression (DeepChem wrapper)
# =========================
# Note: For DeepChem SklearnModel with MLP, we need to handle scaling
# We'll create a pipeline with StandardScaler + MLPRegressor
...
print("\nFitting MLP model on train set...")
